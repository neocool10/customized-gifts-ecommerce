...
function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!errorsObj.error) {
            ...
            if (isValid) {
                ...
                if (token) {
                    (isAuthenticatedIsAdmin() && navigate("/admin/gift"))
                        || (isAuthenticatedIsSuperadmin() && navigate("/superAdmin/getAllUsers"))
                        || (isAuthenticatedIsUser() && navigate("/user/gifts"))
                }
                else {
                    setFailed(true);
                }
    ...
    return (
        <div className='model'>
            ...
            {(isAuthenticatedIsAdmin() && <Navigate to="/admin/gifts" />)
                || (isAuthenticatedIsUser() && <Navigate to="/user/gifts" />)
