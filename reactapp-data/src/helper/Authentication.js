...
export const isSuperAdmin = async (email) => {
    const response = await fetch("http://localhost:8080/isSuperAdmin", {
        method: "POST",
        headers: {
...
export const isUser = async (email) => {
    const response = await fetch("http://localhost:8080/isUser", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
export function signupValidation(props) {
    ...
    if (props.mobileNumber === '') {
        ...
        if (isNaN(props.mobileNumber)) {
            errorsObj.errors.mobileNumber = 'Fiels must contains numbers only';
            errorsObj.status = true;
        }
        else if (props.mobileNumber.length !== 10) {
